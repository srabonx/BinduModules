...
void WavesAndValleys::OnResize(int width, int height)
{
	...
	DirectX::XMMATRIX P = DirectX::XMMatrixPerspectiveFovLH(0.25f * 3.1415926535f, this->GetAspectRatio(), 1.0f, 1000.0f);
...
void WavesAndValleys::BuildShadersAndInputLayout()
{
	...
	m_shaders["standardVS"] = D3DUtil::CompileShader(
		RelativeResourcePath("Shaders/TestModuleColor.hlsl", "TestModules"),
		nullptr, "VS", "vs_5_0");
...
void WavesAndValleys::OnMouseDown(BINDU::MouseButton btn, int x, int y)
{
	...
	SetCapture(this->m_windowHandle);
}
